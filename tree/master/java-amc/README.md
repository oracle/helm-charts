...
| image.repo | Repository for the AMC Docker image | None | Refer 'Create the AMC Docker Image' section in the Deployment Guide |
| image.tag | AMC Docker image tag | None | |
| image.pullPolicy | AMC Docker image pull policy | IfNotPresent | |
| image.pullSecrets | For image pull authentication with Container registry hosting the AMC docker image | None | |
...
| extraEnv.adminServer.USER_MEM_ARGS | User Mem Args to be applied to WebLogic admin server containers  |  -XX:+UseContainerSupport | |
